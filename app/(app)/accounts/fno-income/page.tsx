        ...
        <header className="mt-3 mb-7 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1.5 font-medium text-ink-muted" style={{ fontSize: 15.5 }}>
              F&amp;O income by entity &amp; agency — monthly Rs income across the financial year, with the % return on capital derived automatically.
            </p>
          </div>
        </header>

        <div className="mb-6 flex items-center gap-2.5">
          <Link href={`/accounts/fno-income?fy=${fyStartYear - 1}` as Route} aria-label="Previous financial year" className="inline-flex size-9 items-center justify-center rounded-lg border border-hairline-strong bg-white text-ink-soft transition-colors hover:border-[color:var(--color-altus-red)] hover:text-altus-red">
          ...
          <Link href={`/accounts/fno-income?fy=${fyStartYear + 1}` as Route} aria-label="Next financial year" className="inline-flex size-9 items-center justify-center rounded-lg border border-hairline-strong bg-white text-ink-soft transition-colors hover:border-[color:var(--color-altus-red)] hover:text-altus-red">
            <ChevronRight size={18} strokeWidth={2.4} />
          </Link>
          {!isCurrentFy && (
            <Link href={"/accounts/fno-income" as Route} className="ml-1 inline-flex items-center rounded-lg px-3 py-2 text-[13px] font-bold text-ink-soft hover:text-altus-red">Jump to this FY</Link>
          )}
        </div>
