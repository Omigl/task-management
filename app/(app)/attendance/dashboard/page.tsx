...
export default async function AttendanceDashboardPage({ searchParams }: PageProps) {
  ...
  // Which source is authoritative for this month (HR sheet vs app punches) is
  // decided in ONE place — lib/attendance/month-report.ts — shared with the
  // XLSX + PDF export routes so the screen and the downloads never disagree.
  let rows: DashboardRow[];
  let loadError = false;
  try {
    rows = await getMonthReportRows(year, month, todayISO);
  } catch (err) {
