...
import { formatWeekShort, istYmd } from "@/lib/weekly-goals/week";
...
export interface ReviewItem {
  ...
  code: string | null;
  /**
   * The RAW canonical period key this row sits in — the value the per-card
   * period dropdowns filter on. Shape follows the level:
   *   yearly → '2026' · quarterly → '2026-Q2' · monthly → '2026-07'
   *   weekly → the Monday 'YYYY-MM-DD' · daily → the plan date 'YYYY-MM-DD'.
   */
  periodKey: string;
...
export interface ReviewData {
  ...
  viewedName: string;
  roster: RosterMember[];
  /** Today in IST (`yyyy-mm-dd`) — resolved on the server so the period pickers
   *  can mark "Today" / "This week" without a hydration mismatch. */
  todayYmd: string;
