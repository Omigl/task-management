...
/** Focus ring drawn INSIDE the card body button (it has no border of its own),
 *  white on the selected red card and brand-red on the resting white ones. */
const cardFocusRing = (activeCard: boolean) =>
  `focus-visible:ring-2 focus-visible:ring-inset ${
    activeCard
      ? "focus-visible:ring-white/70"
      : "focus-visible:ring-[var(--color-altus-red)]/55"
  }`;
...
export function ReviewWorkbench({ data }: { data: ReviewData }) {
  ...
  // Each card remembers its OWN period pick ("" = the whole FY), so switching
  // levels never disturbs another card's selection.
  const [periodByLevel, setPeriodByLevel] = React.useState<Record<ReviewLevel, string>>({
    daily: "",
    weekly: "",
    monthly: "",
    quarterly: "",
  ...
  const periodOptions = React.useMemo(() => {
    const out = {} as Record<ReviewLevel, PeriodOption[]>;
    for (const l of LEVELS) out[l.key] = periodOptionsOf(l.key, data.levels[l.key], data.todayYmd);
    return out;
  }, [data.levels, data.todayYmd]);

  /** The pick, self-healing: a period that vanished on refresh falls back to All. */
  const pickedPeriod = React.useCallback(
    (l: ReviewLevel) => {
      const cur = periodByLevel[l];
      return cur && periodOptions[l].some((o) => o.value === cur) ? cur : "";
    },
    [periodByLevel, periodOptions],
  );

  /** Rows for every level narrowed to that card's period — feeds the badges,
   *  the metrics strip, the score maths and the table in one pass. */
  const shownByLevel = React.useMemo(() => {
    const out = {} as Record<ReviewLevel, ReviewItem[]>;
    for (const l of LEVELS) {
      const picked = pickedPeriod(l.key);
      const rows = data.levels[l.key];
      out[l.key] = picked ? rows.filter((i) => i.periodKey === picked) : rows;
    }
    return out;
  }, [data.levels, pickedPeriod]);

  const items = shownByLevel[level];
  const active = LEVELS.find((l) => l.key === level)!;
  const activePeriod = pickedPeriod(level);
  const activePeriodLabel = activePeriod
    ? periodOptionLabel(level, activePeriod, data.todayYmd)
    : null;
    ...
    <div className="flex flex-col gap-4">
      ...
      <nav aria-label="Review level" className="wg-rise grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-5">
        {LEVELS.map(({ key, label, caption, Icon }) => {
          ...
          // Badge tracks THIS card's period pick, so it always matches the rows
          // the dropdown under it selects.
          const count = shownByLevel[key].length;
          const options = periodOptions[key];
          const picked = pickedPeriod(key);
          return (
            <div
              key={key}
              className={`wg-sheen group relative flex flex-col overflow-hidden rounded-2xl transition-[transform,box-shadow,border-color] duration-200 motion-reduce:transition-none ${
            ...
            >
              {/* The card body is the level switch. It's a sibling of the period
                  Select (never its ancestor) — a button inside a button is
                  invalid markup and swallows the dropdown's clicks. */}
              <button
                type="button"
                onClick={() => setLevel(key)}
                aria-pressed={activeBtn}
                className={`flex w-full flex-1 cursor-pointer flex-col items-start gap-1 px-4 pt-3.5 pb-2.5 text-left transition-opacity duration-200 outline-none active:opacity-90 motion-reduce:transition-none ${cardFocusRing(activeBtn)}`}
              >
              ...
              {/* ── the card's own period selector, seated at the bottom interior ── */}
              <div className="mt-auto px-3 pb-3">
                <div
                  className="pt-2.5"
                  style={{
                    borderTop: `1px solid ${
                      activeBtn ? "rgba(255,255,255,0.26)" : "var(--color-hairline)"
                    }`,
                  }}
                >
                  <Select
                    value={picked}
                    // Picking a period also focuses that level: choosing "Q2"
                    // means Q2's rows are what should be on screen.
                    onValueChange={(v) => {
                      setLevel(key);
                      setPeriodByLevel((prev) => ({ ...prev, [key]: v }));
                    }}
                    options={options}
                    disabled={options.length < 2}
                    searchable={key === "daily" || key === "weekly"}
                    searchPlaceholder={key === "daily" ? "Search a date…" : "Search a week…"}
                    ariaLabel={`${label} period to review`}
                    unstyled
                    className={`cursor-pointer rounded-lg border px-2 py-1.5 text-[11.5px] font-bold transition-colors motion-reduce:transition-none ${
                      activeBtn
                        ? "border-white/30 bg-white/15 text-white hover:bg-white/25 [&>svg]:text-white/75"
                        : "border-hairline-strong bg-[color-mix(in_srgb,var(--color-ink-strong)_3.5%,transparent)] text-ink-soft hover:border-[color:color-mix(in_srgb,var(--color-altus-red)_30%,transparent)] hover:text-altus-red-deep"
                    }`}
      ...
      <p className="text-[12.5px]" style={{ color: "var(--color-ink-muted)" }}>
        ...
        — reviewing {data.viewedName}'s {level} items for{" "}
        {activePeriodLabel ? (
          <>
            <span className="font-semibold" style={{ color: "var(--color-altus-red-deep)" }}>
              {activePeriodLabel}
            </span>{" "}
            ·{" "}
          </>
        ) : null}
        FY {data.fyStartYear}–{String((data.fyStartYear + 1) % 100).padStart(2, "0")}
        {level === "daily" ? " (self-completed; no approval tier)" : ""}.
      </p>

      {/* ── (2) metrics summary bar — scoped to the card's selected period ── */}
