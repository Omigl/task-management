...
export function InlineDoerCell({
  ...
  // Text-only name — the circular initials avatar was dropped from the table's
  // Doer column (and its picker) so the column reads as plain names.
  const display = name ? (
    <span className="text-ink-strong font-bold truncate" style={{ fontSize: 15 }}>
      {name}
    </span>
  ) : (
