...
function buildColumns(
  ...
  return [
    ...
    {
      id: "actions",
      // Named header ("Manage") rather than a screen-reader-only label, so the
      // pinned ⋯ column isn't an unlabelled gap at the end of the header row.
...
// #12 — task title with a hover-to-preview card. After ~300ms of hovering the
// title, a popover shows a quick overview of the whole row: task no. + full
// title, the complete description (the cell truncates at 32ch), and a small
// meta grid with status, priority, doer/client/subject and the created / due
// dates. Uses Radix Tooltip (portals out of the table's overflow, and handles
// positioning + delay for free). Always shown — even a short, description-less
// title has status/priority/dates worth surfacing without opening the task.
/**
...
// One label/value line in the hover card's meta grid. `value` is a node so a
// row can carry a coloured chip (status) rather than plain text.
function PreviewRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <>
      ...
      <dd style={{ fontSize: 13.5, color: "var(--color-ink-strong)", fontWeight: 600 }}>
        {value}
      </dd>
    </>
  );
...
function TaskTitleCell({
  ...
  const tone = statusTones[row.status] || STATUS_TONES_FALLBACK[row.status];
          ...
          >
            {row.taskNo != null && (
              <div
                ...
                style={{ fontSize: 12, fontWeight: 700, color: "var(--color-ink-subtle)", marginBottom: 2 }}
              >
                #{row.taskNo}
              </div>
            )}
            ...
            {desc ? (
              <p
                ...
                style={{ fontSize: 14.5, lineHeight: 1.55, color: "var(--color-ink-soft)" }}
              >
                {desc}
              </p>
            ) : (
            ...
            {/* Quick overview — the same facts the row shows, plus the ones its
                narrow columns can't fit, so you never open the task just to
                check a date. */}
            <dl
            ...
            >
              <PreviewRow
                ...
                value={
                  <span
                    className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-pill"
                    style={{
                      fontSize: 12.5,
                      fontWeight: 700,
              ...
              <PreviewRow
                label="Priority"
                value={p === "imp_urgent" ? <CriticalBadge /> : PRIORITY_LABELS[p]}
              />
              <PreviewRow label="Doer" value={row.doerName?.trim() || "Unassigned"} />
              <PreviewRow label="Client" value={row.client?.trim() || "—"} />
              <PreviewRow label="Subject" value={subject || "—"} />
              <PreviewRow
                label="Created"
                value={<span className="tabular-nums">{safeFormat(row.createdAt)}</span>}
              />
              <PreviewRow
                label="Due"
                value={
                  <span
                    className="tabular-nums"
                    style={{
                      color: URGENCY_COLOR[urgency.level],
                      fontWeight: urgency.level === "overdue" || urgency.level === "today" ? 700 : 600,
                    }}
                  >
              ...
              <PreviewRow
                label="Age"
                value={<span className="tabular-nums">{row.ageDays}d</span>}
              />
            </dl>
...
function TaskCard({
  ...
  return (
    ...
    >
      ...
      {/* Text-only doer name — matches the desktop Doer column, which no
          longer shows the circular initials avatar. */}
