...
const fetchTaskTotals = unstable_cache(
  async (): Promise<{ activeTasks: number; archivedTasks: number }> => {
    try {
      ...
      console.error("fetchTaskTotals: DB query failed", err);
...
export async function getNavCounts(args?: {
  ...
  try {
    const [totals, inboxUnread] = await Promise.all([
      fetchTaskTotals().catch(() => ({ activeTasks: 0, archivedTasks: 0 })),
      args?.userId ? getUnreadCount(args.userId).catch(() => 0) : Promise.resolve(0),
    ]);
    return { ...totals, inboxUnread };
  } catch (err) {
    console.error("getNavCounts: failed", err);
    return { activeTasks: 0, archivedTasks: 0, inboxUnread: 0 };
  }
}
